...
The jvm.config is in JRun4in. Change address=8000 to 8001 and _save as_ new config file, with new name, example **jvm.config_FD1**
...
jrunsvc -install wfs1 "Macromedia CFMX AS wfs1" "Macromedia CFMX AS wfs1" -config jvm.config_wfs1\_
...
to the "Arguments to VM" section: _\-Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=8000_, and restart the server.

**Be extra carefull with spaces and returns.**

Full Example of section:  
_\# Arguments to VM  
java.args=-server -Xmx512m -Dsun.io.useCanonCaches=false -XX:MaxPermSize=192m -Djava.awt.headless=true -Dcoldfusion.rootDir={application.home}/ -Djava.compiler=NONE -Xnoagent -Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=8000_
