window.addEventListener('DOMContentLoaded', (event) => {
    ...
    if (navToggle && navLinks) {
        ...
        function trapFocus(element) {
            var all = element.querySelectorAll('a, button, input, textarea, select');
            var visible = [];
            for (var idx = 0; idx < all.length; idx++) {
                if (all[idx].tabIndex !== -1) visible.push(all[idx]);
            }
            var first = visible[0];
            var lastIdx = visible.length - 1;
            var last = visible[lastIdx];
            function handler(e) {
                if (e.key === 'Tab') {
                    if (e.shiftKey) {
        ...
        toggle.addEventListener('click', function(e) {
            ...
            dropdownToggles.forEach(function(other) {
                if (other !== toggle) {
                    ...
                    var m = other.closest('.nav-dropdown').querySelector('.dropdown-menu');
    ...
    if (progressBars.length > 0) {
        function animateBar(bar) {
            ...
            if (span && targetWidth) {
                ...
                requestAnimationFrame(function() { span.style.width = targetWidth + '%'; });
            }
        }
        if ('IntersectionObserver' in window) {
            ...
            progressBars.forEach(function(bar) { obs.observe(bar); });
        } else {
...
window.copyCode = function(button) {
    ...
    if (navigator.clipboard && window.isSecureContext) {
        navigator.clipboard.writeText(text).then(function() { showCopySuccess(button); });
    } else {
...
function fallbackCopy(text, button) {
    ...
    ta.focus();
    ta.select();
    try { document.execCommand('copy'); showCopySuccess(button); } catch (e) {}
